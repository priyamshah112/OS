class Process:
    def __init__(self, p_no, at, bt,wt,tat,nt,ct,rt,priority):
        self.p_no = p_no
        self.at = at
        self.bt = bt
        self.wt =wt
...
def Shift(alist):
  alist.sort(key=lambda x:x.priority)
  return alist
...
def main():
  ...
  for i in range(0,n):
    pt.insert(i,Process(i,int(input("Enter Arrival Time : ")),int(input("Enter Burst Time :")),0.0,0.0,0.0,0,0,int(input("Enter priority : "))))
    pt[i].rt=pt[i].bt
